...
var makeCreateModel = function (hashFn) { return function (schema, _grid, rowIdxs) {
    var changes = [];
    var grid;
    var lGetGrid = function (options) {
        if (options.applyUnsavedUpdates) {
            var patch = changes.flatMap(function (change) {
                return change.__metadata.rowIdx.map(function (ridx) { return ({
                    rowIdx: ridx - 1,
                    colIdx: getFieldFromSchema(change.fieldname, schema).__metadata.idx,
                    newValue: change.value.to,
                }); });
            });
            var newGrid_1 = Array.from(grid || []);
            patch.forEach(function (p) {
                newGrid_1[p.rowIdx][p.colIdx] = p.newValue;
            });
            return newGrid_1;
        }
        return grid;
    };
    var makeToJSON = function (phashFn, groupingIdxs) { return function (pschema, pgrid) {
        return utils_1.makeToJSONWithSchema(phashFn)(pschema, pgrid, groupingIdxs);
    }; };
    ...
    var model = {
        getAll: function (options) {
            ...
            return makeToJSON(hashFn, rowIdxs)(schema, lGetGrid(options));
        },
        get: function (filter, options) {
            if (options === void 0) { options = { applyUnsavedUpdates: true }; }
            return createGetOne(schema, lGetGrid(options), makeToJSON(hashFn, rowIdxs))(filter);
        },
        getById: function (id) {
            return createGetById(schema, grid, makeToJSON(hashFn, rowIdxs))(id);
        },
        ...
        filter: function (filterFn) {
            return createFilter(schema, grid, makeToJSON(hashFn, rowIdxs))(filterFn);
        },
